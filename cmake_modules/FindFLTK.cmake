...
IF(UNIX)
  ...
  # Look for the headers in common locations to save having to specify them
  # manually.
  FIND_PATH( FLTK_DIR Fl.H  /usr/include/FL /usr/local/include/FL  )
ENDIF(UNIX)

IF(APPLE)
  SET( FLTK_PLATFORM_DEPENDENT_LIBS  "-framework Carbon -framework AGL -framework Cocoa -framework ApplicationServices -lz")
  # Look for the headers in common locations to save having to specify them
  # manually.
  FIND_PATH( FLTK_DIR Fl.H  /opt/local/include/FL /usr/local/include/FL /usr/include/FL   )
ENDIF(APPLE)
...
IF(NOT FLTK_DIR)
  ...
  FIND_PATH(FLTK_DIR FLTKConfig.cmake
    ...
    FIND_PATH(FLTK_DIR NAMES include/FL/Fl.h include/FL/Fl.H bin/fltk-config bin/fluid lib/libfltk.a PATHS
...
IF(FLTK_DIR)
  ...
  ELSE(FLTK_BUILT_WITH_CMAKE)
    ...
    FIND_PROGRAM(FLTK_FLUID_EXECUTABLE NAMES fluid PATHS ${FLTK_INCLUDE_DIR}/fluid ${FLTK_INCLUDE_DIR} ${FLTK_DIR} ${FLTK_DIR}/fluid NO_DEFAULT_PATH)
    FIND_PROGRAM(FLTK_FLUID_EXECUTABLE NAMES fluid )
    ...
    FIND_PATH(FLTK_INCLUDE_DIR NAMES FL/Fl.h FL/Fl.H PATHS ${FLTK_INCLUDE_SEARCH_PATH} NO_DEFAULT_PATH)
    FIND_PATH(FLTK_INCLUDE_DIR NAMES FL/Fl.h FL/Fl.H)
    ...
    FIND_LIBRARY(FLTK_BASE_LIBRARY NAMES fltk fltkd
      PATHS ${FLTK_LIBRARY_SEARCH_PATH} NO_DEFAULT_PATH)
    FIND_LIBRARY(FLTK_BASE_LIBRARY NAMES fltk fltkd)
    ...
    IF(UNIX)
      FIND_PROGRAM(FLTK_CONFIG_SCRIPT fltk-config PATHS ${FLTK_DIR} ${FLTK_BIN_DIR} NO_DEFAULT_PATH)
...
IF(FLTK_FOUND)
  ...
  IF(UNIX)
    # Look for the Xft library, which is used by some of the
    # fltk based programs
    FIND_LIBRARY(XFT_LIB NAMES Xft PATHS ${X11_LIB_SEARCH_PATH})
  
    SET(FLTK_LIBRARIES ${FLTK_LIBRARIES} ${XFT_LIB})
  ENDIF(UNIX)
...
IF( FLTK_FOUND )
    IF( NOT FLTK_FIND_QUIETLY )
        MESSAGE( STATUS "Looking for FLTK headers -- found" )
        MESSAGE( STATUS "Looking for FLTK libraries -- found" )
    ENDIF( NOT FLTK_FIND_QUIETLY )
ELSE( FLTK_FOUND )
    IF( FLTK_REQUIRED )
        MESSAGE( FATAL_ERROR "Could NOT find FLTK -- please provide the directory containing MVL.")
    ELSE( FLTK_REQUIRED)
        MESSAGE( STATUS "Looking for FLTK library -- not found" )
    ENDIF( FLTK_REQUIRED)
ENDIF( FLTK_FOUND )
