 ...
 * SharedPtr.h
 *
 * Derived from Poco::SharedPtr, but allows ReleasePolicies to have
 * state.
 *
...
public:
    ...
    SharedPtr(const RP& releasePolicy): _releasePolicy(releasePolicy), _pCounter(new RC), _ptr(0)
    {
    }

    SharedPtr(C* ptr, const RP& releasePolicy): _releasePolicy(releasePolicy), _pCounter(new RC), _ptr(ptr)
    {
    }

    template <class Other, class OtherRP>
    SharedPtr(const SharedPtr<Other, RC, OtherRP>& ptr): _releasePolicy(ptr._releasePolicy), _pCounter(ptr._pCounter), _ptr(const_cast<Other*>(ptr.get()))
    {
        _pCounter->duplicate();
    }

    SharedPtr(const SharedPtr& ptr): _releasePolicy(ptr._releasePolicy), _pCounter(ptr._pCounter), _ptr(ptr._ptr)
    {
        _pCounter->duplicate();
    }
    ...
    /**
     * This is potentially dangerous if the current ReleasePolicy is not
     * compatible with the pointer provided.
     * @param ptr
     * @return
     */
    ...
    {
        std::swap(_releasePolicy, ptr._releasePolicy);
